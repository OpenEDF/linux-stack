...
static irqreturn_t gpio_irq_handler(int irq,void *dev_id)
{
    ...
    pm_wakeup_event(dev_id, 0); // Inform PM core of the wake-up even
...
static int __init etx_driver_init(void)
{
  ...
  */
  ...
  if (request_irq(GPIO_irqNumber,             //IRQ number
    ...
    /* register the gpio irq as wakeup source */
    ret = enable_irq_wake(GPIO_irqNumber)

    if (ret){
        pr_err("my_device: cannot register IRQ as wakrup sorces, ret = %d\n", ret);
        goto r_gpio_in;
    }
